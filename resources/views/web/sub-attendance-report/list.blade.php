<!-- View stored in resources/views/web/sub-attendance-report/list.blade.php -->
...
@section('content')
    @php
        ...
        $action = $isManagerRoute
                ? isset($user)
                    ? "/departments/users/{$user->id}/sub-attendance-reports"
                    : "/departments/sub-attendance-reports"
        ...
        @if ($isManagerRoute)
            @if (isset($user))
                ...
                @include('web.common.navbar', [
                    'page' => 'sub-attendance-report',
                    'id' => $user->id,
                ])
            @else
            <div class="row">
                    <div class="col">
                        <h3 class="mt-3">{{ Auth::user()->department->name }} 假日上班申請列表</h3>
                    </div>
                </div>
            @endif
        @else
            <div class="row">
                <div class="col">
                    <h3 class="mt-3">{{ $user->name }}（{{ $user->alias }}）假日上班申請列表</h3>
                </div>
            </div>
        @endif
            ...
            <tbody>
                @foreach($reports as $report)
                    <tr>
                        ...
                        <td>{{ $report->date }}</td>
                        <td>{{ $report->reason }}</td>
                        <td>{{ \App\Enums\CompensationType::getDescription($report->compensation) }}</td>
                        <td>
                            @if ($report->permitted)
                                許可
    ...
    <script>
        ...
        $('#approve').on('click', () => {
            ...
            const url = '/departments/sub-attendance-reports/' + id;
            axios
            .put(url, {
                'note': note,
                'status': status
